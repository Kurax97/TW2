...
<body>
  ...
  <form action="factureStarWars.php" method = "get">
    <fieldset>
      ...
      <label for="cp">Code postal</label><input type="text"  id="cp" name="cp" size="6" maxlength="6" minlength="5" required="required"/>
      <label for="ville">Ville</label><input type="text"  id="ville" name="ville" size="25" maxlength="100" required="required"/><br />
      <label for="adhesion">Adhesion:</label>
      <select name="adhesion">
        <option value="oui">oui</option>
        <option value="non">non</option>
        <option value="dejaMembre">Déjà Membre</option>
      </select>
      <p>Choisi tes figures:</p>
      <?php 
      ...
      echo "<div>";
      foreach (FIG_VALUES as $key => $value) {
        echo '<input type="checkbox" name="fig[]" value="'.$value.'"><label>'.$value.'</label>';
      }
      echo "</div>";
      ?>
      <br>
